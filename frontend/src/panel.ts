...
export function renderSupplierCartPanel(
  ...
  const panelContext = (data?.context as PanelContext | undefined) ?? {};
  ...
  const transferButtonEl = transferButton;
  const openButtonEl = openButton;
  const loaderEl = loader;
  const resultEl = result;
  const cartKeyEl = cartKey;
  ...
  function applyCartData(cartData: CartData | undefined): void {
    if (!cartData) {
      return;
    }
