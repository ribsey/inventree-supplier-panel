...
class Digikey:
    ...
    def get_digikey_partdata_v4(self, sku, options):
        ...
        country_code = self.COUNTRY_CODES.get(
            InvenTreeSetting.get_setting("INVENTREE_DEFAULT_CURRENCY"),
        ...
        header = {
            ...
            "X-DIGIKEY-Locale-Currency": InvenTreeSetting.get_setting(
        ...
        try:
            if response_json["status"] != 200:
                part_data["error_status"] = (
                    response_json["title"] + response_json["detail"]
                )
                return part_data
        except KeyError:
            pass
        ...
        part_data["lifecycle_status"] = response_json["Product"]["ProductStatus"][
            "Status"
        ]
        part_data["description"] = response_json["Product"]["Description"][
            "DetailedDescription"
        ]
    ...
    def create_digikey_cart(self, order):
        ...
        while not Digikey.check_valid_listname(self, list_name):
            ...
            if i == version + 20:
                cart_data["ID"] = ""
                cart_data["error_status"] = (
                    "No valid list name found within 20 attempts"
                )
                return cart_data
        ...
        url_data = {"ListName": list_name, "accept": "application/json"}
        response = Wrappers.post_request(
            self, json.dumps(url_data), url, headers=header
        )
    ...
    def update_digikey_cart(self, order, list_id):
        ...
        for item in order.lines.all():
            cart_items.append(
                {
                    "RequestedPartNumber": item.part.SKU,
                    "Quantities": [
                        {
                            "Quantity": int(item.quantity)
                            * int(item.part.pack_quantity_native)
                        }
                    ],
            ...
            if p["DigiKeyPartNumber"] != "":
                ...
                for pack_option in p["Quantities"][0]["PackOptions"]:
                    if pack_option["DigiKeyPartNumber"] == p["RequestedPartNumber"]:
                        break
                if (
                    pack_option["MinimumOrderQuantity"]
                    > p["Quantities"][0]["QuantityRequested"]
                ):
                    cart_items.append(
                        {
                            "SKU": pack_option["DigiKeyPartNumber"],
                            "IPN": p["CustomerReference"],
                            "MPN": "",
                            "Manufacturer": "",
                            "Description": "",
                            "QuantityRequested": p["Quantities"][0][
                                "QuantityRequested"
                            ],
