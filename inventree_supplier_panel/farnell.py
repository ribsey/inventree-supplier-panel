...
class Farnell:
    ...
    def get_farnell_partdata(self, sku, options):
        ...
        path_string = (
            path
            + "versionNumber=1.5&term=id:"
            + sku
            + "&storeInfo.id="
            + store
            + "&resultsSettings.responseGroup=large&callInfo.responseDataFormat=json&callinfo.apiKey="
            + access_key
        )
        ...
        if response["numberOfResults"] == 0:
            part_data["error_status"] = (
                f'Part with SKU "{sku}" not found in Farnell catalog!'
            )
            return part_data
        ...
        part_data["package"] = (
            str(response["products"][0]["translatedMinimumOrderQuality"])
            + " "
            + response["products"][0]["unitOfMeasure"]
        )
        for pb in response["products"][0]["prices"]:
            new_price = pb["cost"]
            part_data["price_breaks"].append(
                {"Quantity": pb["from"], "Price": new_price, "Currency": currency}
            )
        return part_data
