...
class Wrappers:
    ...
    def post_request(self, post_data, path, headers):
        ...
        if proxy_con and proxy_url:
            ...
            self.get_setting("PROXY_CON") != "" and self.get_setting("PROXY_URL") != ""
        ):
        ...
        try:
            response = requests.post(
                path, proxies=proxies, data=post_data, timeout=15, headers=headers
            )
    ...
    def get_request(self, path, headers):
        ...
        if proxy_con and proxy_url:
            ...
            proxies = {}
        try:
            response = requests.get(path, proxies=proxies, timeout=15, headers=headers)
        except requests.exceptions.RequestException as e:
            self.status_code = e.args
            raise ConnectionError
        return response
