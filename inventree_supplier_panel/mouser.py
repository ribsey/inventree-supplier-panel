...
class Mouser:
    ...
    def get_mouser_partdata(self, sku, options):
        ...
        url = (
            ...
            + self.get_setting("MOUSERSEARCHKEY")
        )
    ...
    def get_mouser_package(self, part_data):
        ...
        for att in attributes:
            if (
                att["AttributeName"]
                == att_names["packaging"][self.get_setting("MOUSERLANGUAGE")]
            ):
    ...
    def update_mouser_cart(self, order, cart_key):
        ...
        for item in order.lines.all():
            cart_items.append(
                {
                    "MouserPartNumber": item.part.SKU,
                    "Quantity": int(item.quantity),
                    "CustomerPartNumber": item.part.part.IPN,
                }
            )
        cart = {"CartKey": cart_key, "CartItems": cart_items}
        url = (
            "https://api.mouser.com/api/v001/cart/items/insert?apiKey="
            + self.get_setting("MOUSERCARTKEY")
            + "&countryCode="
            + country_code
        )
        header = {"Content-type": "application/json", "Accept": "application/json"}
        ...
        shopping_cart = {
            "MerchandiseTotal": response["MerchandiseTotal"],
